...
const RUN_TS = join(ROOT, "scripts", "run.ts");
...
function runSkill(args: string[]): string {
  return execFileSync("npx", ["tsx", RUN_TS, ...args], {
    cwd: ROOT, encoding: "utf-8", timeout: 30_000,
  });
}

// ─── scripts/run.ts — the main entry point ──────────────────────────────────
//
// This is how an agent uses the project: read SKILL.md, call scripts/run.ts.

describe("scripts/run.ts", () => {
  it("lists available skills", () => {
    const out = runSkill(["list"]);
  ...
  it("runs qr-code", () => {
    const svg = runSkill(["qr-code", '{"data":"hello"}']);
    expect(svg).toContain("<svg");
    expect(svg).toContain("<rect");
  });

  it("runs pdf-builder", () => {
    const json = JSON.parse(runSkill(["pdf-builder", '{"title":"Test","body":["Hello."]}']));
    expect(json.base64).toBeTruthy();
    expect(json.pages).toBe(1);
  });

  it("runs spreadsheet-builder", () => {
    const csv = runSkill(["spreadsheet-builder", '{"headers":["A","B"],"rows":[["1","2"]]}']);
    expect(csv).toContain("A,B");
    expect(csv).toContain("1,2");
  });

  it("runs chart-generator", () => {
    const svg = runSkill(["chart-generator", '{"type":"pie","data":{"X":60,"Y":40}}']);
    expect(svg).toContain("<svg");
    expect(svg).toContain("<path");
  });

  it("runs ical-generator", () => {
    const ics = runSkill(["ical-generator", '{"events":[{"summary":"Test","start":"2025-03-15T14:00:00","end":"2025-03-15T15:00:00"}]}']);
...
describe("SKILL.md", () => {
  it("exists at repo root with valid frontmatter", () => {
    const content = readFileSync(join(ROOT, "SKILL.md"), "utf-8");
    expect(content).toContain("name: ai-skills");
    expect(content).toContain("description:");
    expect(content).toContain("scripts/run.ts");
  });

  it("references all 5 skills", () => {
    const content = readFileSync(join(ROOT, "SKILL.md"), "utf-8");
    expect(content).toContain("qr-code");
    expect(content).toContain("pdf-builder");
    expect(content).toContain("spreadsheet-builder");
    expect(content).toContain("chart-generator");
    expect(content).toContain("ical-generator");
  });
});
